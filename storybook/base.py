...
def brainstorm_and_set(
    ...
    retries: int = 3, list: bool = False, multiple: bool = False) -> str:
    """
    ...
    while r.strip().lower() == 'more':
        ...
        if len(text.strip()) == 0:
            ...
            if retry >= retries:
                logging.error("#######\nStart Prompt\n#######\n{}".format(start_prompt))
                return Exception("Too many retries")
            continue
        if list:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        answer = binary_query(start_prompt, question_antag_prompt, verbose=True)
        if answer is None or not answer:
            print("\nNow we'll brainstorm some antagonist ideas. Generating...\n")
            start_prompt = brainstorm_and_set(start_prompt, antag_ideas_prompt, "Antagonist idea", params, list=True)

        print("\nGreat, now we'll develop the antagonist a bit more. What do you think about this...\n")
        start_prompt = brainstorm_and_set(start_prompt, develop_antag_prompt, "Antagonist", develop_params)

        # brainstorm and enumerate supporting characters
        print(start_prompt)
        print("\nNow we'll brainstorm some supporting characters. Generating...\n")
        start_prompt = brainstorm_and_set(start_prompt, char_ideas_prompt, "Supporting characters", params, list=True, multiple=True)
        print(start_prompt)
